...
TcpTimeStepGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  // Store the actual multiplier, not a scaled absolute value
  m_actionMultiplier = mult;
  NS_LOG_INFO ("MyExecuteActions: " << action << " multiplier=" << mult);
  return true;
}
...
TcpTimeStepGymEnv::GetObservationSpace()
{
  ...
  float low = 0.0;
  float high = 1e9;
  std::vector<uint32_t> shape = {paramNum,};
...
TcpTimeStepGymEnv::GetObservation()
{
  ...
  float cwnd = m_tcb ? static_cast<float>(m_tcb->m_cWnd.Get()) : 0.0f;
  box->AddValue(cwnd);

  float srtt = m_tcb ? static_cast<float>(m_tcb->m_srtt.Get().GetSeconds() * 1000.0) : 0.0f;
  box->AddValue(srtt);

  // loss rate placeholder – we'll compute in Python or improve later
  box->AddValue(0.0f);

  // M4.1 FIX: compute real delivered throughput from receiver's PacketSink
  double stepSec = m_timeStep.GetSeconds();
  float throughputBps = 0.0f;

  // Search all nodes for a PacketSink application
          ...
          if (sink)
            {
              ...
              if (stepSec > 0)
                {
                  throughputBps = static_cast<float>((deltaBytes * 8.0) / stepSec);
                }
              break;
            }
        }
...
TcpTimeStepGymEnv::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  // action
  // tcb->m_cWnd = m_new_cWnd;
  // Apply the action multiplier to the current window
