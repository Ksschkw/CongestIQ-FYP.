"""eval_m3.py – Run the trained RL agent against CUBIC and compare metrics."""
...
# Add reward wrapper path (not used here but keep for reference)
...
def main():
    # 1. Connect to the environment (this will launch ns-3 automatically)
    from ns3gym import ns3env
    env = ns3env.Ns3Env(port=5555, stepTime=0.1, startSim=True, simSeed=1, simArgs={}, debug=False)

    # Wrap to convert old Gym spaces to Gymnasium
    class EvalEnv(gym.Env):
        def __init__(self, raw_env):
            ...
            self.observation_space = gym.spaces.Box(
                ...
                shape=raw_env.observation_space.shape,
                dtype=np.float32)
    ...
    # 4. Parse FlowMonitor output (produced in ns-3-dev directory)
    xml_file = "/home/ksschkw/Projects/fyp/ns-3-dev/rl-vs-cubic.flowmon"
    if os.path.exists(xml_file):
        flows = parse_flowmon(xml_file)
        if flows:
            print("\nFlow comparison (RL vs CUBIC):")
            for f in flows:
                print(f"  Flow {f['flowId']}: Throughput={f['throughput_kbps']:.1f} kbps, "
                      f"Delay={f['mean_delay_ms']:.2f} ms, Loss={f['loss_ratio']*100:.2f}%")
            # Bar chart
            labels = ['RL (Flow 1)', 'CUBIC (Flow 2)']
            throughputs = [f['throughput_kbps'] for f in flows[:2]]
            plt.bar(labels, throughputs, color=['blue','orange'])
            plt.ylabel('Throughput (kbps)')
            plt.title('M3.6: RL vs CUBIC Throughput')
            plt.savefig('/home/ksschkw/Projects/fyp/docu/m3-single-agent-rl/results/m3_comparison.png')
            plt.close()
            print("Comparison chart saved.")
        else:
            print("No data flows found in XML.")
    else:
        print(f"FlowMonitor file not found: {xml_file}")
